(1) Identify the specific process from the context as 'Automated Missing Invoice Reconciliation' (NAV Online Számla vs. Received PDF) for the Hungarian market. (2) Design the system architecture strictly using the specified tools: Email (e.g., Gmail), Spreadsheet (e.g., Google Sheets), and a Generic Automation Tool (e.g., Make/Zapier). (3) Define the Triggers: Establish a time-based trigger (e.g., daily schedule) for reconciliation and an event-based trigger (e.g., incoming email) for document ingestion. (4) Specify the Data that must be captured: List essential fields like Invoice Number (Számlaszám), Vendor Name, Net/Gross Amount, Date, and Processing Status. (5) Map out Rules and Branches: (a) Ingestion Branch: If email has PDF -> Save to Drive -> Log in 'Received' Sheet. (b) Reconciliation Branch: Compare 'NAV Data' Sheet vs. 'Received' Sheet. (c) Action Logic: If Invoice exists in NAV but not in Received -> Send 'Missing Invoice' email to Vendor. (6) Define When humans step in: Scenarios like unrecognized vendors, unparseable attachments, or dispute replies. (7) Define What happens when something fails: Handling email bounces, API timeouts, or data format errors. (8) Draft a Plain Language Description: Create a precise, step-by-step prompt that a non-technical user could paste into an AI or give to a no-code developer to build this exact system.

Based on the attached market analysis document, I'll design a comprehensive Automated Missing Invoice Reconciliation System for the Hungarian market using Gmail, Google Sheets, and Make/Zapier.
...
Business Problem: Hungarian SMEs receive invoice data from NAV Online Számla API (XML format) but often don't receive the actual PDF invoices from vendors. This creates:
...
Send notification: "Vendor {Name} has not responded after 3 automated emails. Consider phone call or alternative contact."
...
Tracks invoices that Hungarian tax authority (NAV) says exist

Monitors which actual PDF invoices our company has received

Automatically emails vendors when we're missing their invoice PDF

Logs everything so accountants can see what's happening
...
If PDF can't be read (OCR fails) → Add email label "⚠️ Manual Review Needed" and add note in Received_PDFs sheet

If file name already exists in Drive → Add timestamp to make it unique (InvoiceNumber_VendorName_Date_HH-MM.pdf)
...
Keywords indicating problem: "tévedés" (mistake), "mégsem" (cancelled), "rossz cím" (wrong address), "nem mi" (not us)
...
→ Send auto-reply: "Thank you for your response. However, we still need the PDF invoice. Please attach it to your next email."

WHEN HUMANS NEED TO STEP IN:

The automation will notify humans (send email to accounting@company.com) when:

New vendor not in database - automation can't find email address
→ Human must add vendor email to "Vendors" sheet

PDF can't be read - OCR fails or PDF is scanned image with low quality
→ Human must manually type invoice number and amount

Vendor doesn't respond after 3 emails - automation gives up
→ Human should call vendor or find alternative contact

Amount doesn't match - PDF shows different amount than NAV data (>5% difference)
→ Human must investigate (maybe partial payment, credit note, or error)

Technical errors - NAV API down, Google Drive full, Gmail quota exceeded
...
Add module: Gmail > Watch Emails

Configure: Watch label "INBOX", filter "has:attachment filename:pdf"

Add module: Gmail > Get an Email

Add router with 2 paths:

Path 1: If attachment exists → Continue

Path 2: If no attachment → Stop

Add module: Gmail > Download an Attachment

Add module: Google Drive > Upload a File

Add module: Google Cloud Vision > OCR (or Make's "Parse Document")

Add module: Google Sheets > Add a Row (to Received_PDFs)

Add module: Gmail > Add a Label to Email ("✅ Processed")

Add module: Google Sheets > Search Rows (in NAV_Data, by Invoice Number)

Add router:

If found → Update Row (set Status = "PDF Received")

If not found → Do nothing

Add module: Google Sheets > Search Rows (in Missing_Invoices)

Add module: Google Sheets > Delete a Row (if found in Missing_Invoices)

Scenario 2: "Daily Reconciliation"

Add module: Schedule > Every Day (7:00 AM, Europe/Budapest timezone)

Add module: Google Sheets > Get Range Values (NAV_Data, all rows where Status != "PDF Received")

Add module: Iterator (process each row one by one)

Inside iterator:

Add module: Google Sheets > Search Rows (Received_PDFs, match Invoice Number)
...
Add module: Google Sheets > Search Rows (Missing_Invoices, match Invoice Number)

Add router with 3 paths:
...
Add module: Google Sheets > Search Rows (Vendors sheet, match Vendor Tax ID)

Add module: Gmail > Send an Email (use template from Step 4c above)

Add module: Google Sheets > Update a Row (Missing_Invoices, increment counter)

Add module: Google Sheets > Add a Row (Audit_Log)

After iterator ends:

Add module: Gmail > Send an Email (summary report to accounting manager)

Scenario 3: "Handle Replies"

Add module: Gmail > Watch Emails

Configure: Watch "Sent" folder, filter "is:reply to:vendor_email"

Add router with 3 paths:

Path A: If has attachment → Trigger Scenario 1

Path B: If body contains "tévedés|mégsem|rossz cím" → Add label, forward to human

Path C: Otherwise → Send auto-reply requesting attachment
...
That's the complete system! Copy this entire prompt to a no-code developer or AI assistant, and they should be able to build the automation without additional questions.
...
This system addresses the exact pain point identified in your market analysis: the gap between NAV's XML data and companies' physical invoice possession, enabling proactive compliance before the September 2025 penalty enforcement.

⁂

NAV-szamlaegyeztetes-KKV-piac-elemzese.docx
